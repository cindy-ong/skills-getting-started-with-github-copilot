      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length > 0) {
          details.participants.forEach(participant => {
            participantsHTML += `<li class="participant-item"><span class="participant-name">${participant}</span><span class="delete-participant" title="Remove participant" data-activity="${name}" data-participant="${participant}">✖</span></li>`;
          });
        } else {
        ...
        activityCard.querySelectorAll('.delete-participant').forEach(icon => {
          icon.addEventListener('click', async (e) => {
            ...
            if (confirm(`Remove ${participantEmail} from ${activityName}?`)) {
              try {
                const res = await fetch(`/activities/${encodeURIComponent(activityName)}/participants/${encodeURIComponent(participantEmail)}`, {
                  method: 'DELETE'
                });
                if (res.ok) {
